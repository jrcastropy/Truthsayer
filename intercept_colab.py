...
def main(input,
  ...
  DRAW_LANDMARKS = landmarks is not None
  BPM_CHART = bpm is not None
  if ttl:
  ...
  with mp.solutions.face_mesh.FaceMesh(
    ...
    with mp.solutions.hands.Hands(
        ...
        output_filepath = os.path.join(output_dir, RECORDING_FILENAME)
        recording = cv2.VideoWriter(output_filepath, cv2.VideoWriter_fourcc(*'MJPG'), FPS_OUT, FRAME_SIZE)
        while cap.isOpened():
            success, image = cap.read()
            if not success: break
            calibration_frames += process(image, face_mesh, hands, calibrated, DRAW_LANDMARKS, BPM_CHART, False, fps)
...
def add_text(image, tells, calibrated):
  ...
  text_y = TEXT_HEIGHT
  if mood:
    write("Mood: {}".format(mood), image, 0, int(.75 * image.shape[0]))
  if calibrated:
